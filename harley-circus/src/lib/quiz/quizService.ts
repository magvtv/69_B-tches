  ...
  async loadLevel(levelNumber: number): Promise<QuizLevel> {
    ...
    try {
      ...
      if (levelNumber === 0) {
        ...
        // For other levels, use Vite's glob-import to avoid dynamic path resolution issues
        const levelModules = import.meta.glob('../../data/level*.json') as Record<string, () => Promise<{ default: QuizLevel }>>
        const targetPath = `../../data/level${levelNumber}.json`
        const loader = levelModules[targetPath]
        if (!loader) {
          throw new Error(`Level file not found: ${targetPath}`)
        }
        const levelData = await loader()
