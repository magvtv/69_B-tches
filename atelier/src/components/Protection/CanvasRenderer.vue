...
async function renderProtectedImage() {
  ...
  img.onload = () => {
    ...
    let drawWidth = width.value;
    let drawHeight = height.value;
    
    if (aspectRatio > 1) {
      // Landscape
      drawHeight = width.value / aspectRatio;
    } else {
      // Portrait
      drawWidth = height.value * aspectRatio;
    }
    
    const x = (width.value - drawWidth) / 2;
    const y = (height.value - drawHeight) / 2;
  ...
  img.onerror = () => {
    ...
    ctx.fillText('Artwork not available', width.value / 2, height.value / 2);
  };
}
...
function applyVignette(ctx: CanvasRenderingContext2D) {
  ...
  const gradient = ctx.createRadialGradient(
    width.value / 2, height.value / 2, height.value * 0.3,
    width.value / 2, height.value / 2, height.value * 0.8
  );
...
function applySegmentation(ctx: CanvasRenderingContext2D) {
  ...
  const segmentSize = width.value / protectionConfig.value.segmentCount;
  
  ctx.strokeStyle = 'rgba(212, 175, 55, 0.03)';
  ctx.lineWidth = 0.5;
  
  // Draw vertical lines
  for (let x = 0; x < width.value; x += segmentSize) {
    ctx.beginPath();
    ctx.moveTo(x, 0);
  ...
  for (let y = 0; y < height.value; y += segmentSize) {
    ctx.beginPath();
    ctx.moveTo(0, y);
